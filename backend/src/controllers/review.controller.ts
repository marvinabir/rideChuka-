...
export const getAllReviewsForEventController = async (req: Request, res: Response) => {
  ...
  try {
    const reviews = await getAllReviewsForEvent(Number(eventId));
...
export const addReviewForEventController = async (req: Request, res: Response) => {
  ...
  try {
    const review = await addReviewForEvent(Number(userId), Number(eventId), Number(rating), comment);
...
export const updateReviewController = async (req: Request, res: Response) => {
  ...
  try {
    const updatedReview = await updateReview(Number(reviewId), Number(rating), comment);
